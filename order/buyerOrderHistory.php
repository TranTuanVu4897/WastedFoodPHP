...
$query = "SELECT `order`.`id` AS `order_id`,`buyer_id`,`product_id`,`quantity`,
        ...
        `address`,`latitude`,`longitude`,`seller`.`description` AS `seller_description`
...
while ($row = mysqli_fetch_assoc($result)) {
    $seller = new Seller($row['account_id'], null, null, null, null, null, null, null, null, $row['seller_name'], $row['seller_image'], $row['address'], $row['latitude'], $row['longitude'], $row['seller_description'], null);
